...
class ClipAdam(Optimizer):
    """The ClipAdam optimizer.
    TODO: Add descriptions of this optimizer
    ...
    **lazy updates** at step t are applied by::
    ...
    def __init__(self, learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-8,
                 lazy_update=True, **kwargs):
        super(ClipAdam, self).__init__(learning_rate=learning_rate, **kwargs)
        self.beta1 = beta1
        self.beta2 = beta2
        self.epsilon = epsilon
        self.lazy_update = lazy_update
    ...
    def update(self, index, weight, grad, state):
        ...
        grad = grad * self.rescale_grad + wd * weight
        if self.clip_gradient:
            grad = clip(grad, auto_clip)
        mean[:] *= self.beta1
        mean[:] += (1. - self.beta1) * grad
        var[:] *= self.beta2
        var[:] += (1. - self.beta2) * grad * grad

        mean_bar = mean / (1. - coef1)
        var_bar = var / (1. - coef2)

        # update weight
        weight[:] -= lr * mean_bar / (sqrt(var_bar) + self.epsilon)"""
